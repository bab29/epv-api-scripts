...
#This is the specific user who will be adding the safes and accounts.
...
#>
	...
	$bodySafe = @{ SafeName = $safeName; Description = $description; OLACEnabled = $false; ManagingCPM = $CPM_NAME; NumberOfVersionsRetention = $NumberOfVersionsRetention }
...
#>
	param (
		...
		[string]$ownerName,
		[Parameter(Mandatory = $true)]
		[ValidateSet("Admin", "Auditor", "Owner", "EndUser", "Approver", "DefaultAdminGroup", "DefaultAuditGroup","DefaultSafeManagerGroup")]
		[string]$ownerRole,
		[Parameter(Mandatory = $false,
	...
	switch ($ownerRole)
	{
		...
		"DefaultAdminGroup"
		{
			$permListAccounts = $permAddAccounts = $permUpdateAccountContent = $permUpdateAccountProperties = $permInitiateCPMManagement = `
				...
				$permViewAuditLog = $permViewSafeMembers = $permAccessWithoutConfirmation = $true
			$permRequestsAuthorizationLevel = 1
		}
		"DefaultSafeManagerGroup"
		{
			$permManageSafe = $permManageSafeMembers = $permBackupSafe = $permViewSafeMembers = $permAccessWithoutConfirmation = $true
			$permRequestsAuthorizationLevel = 1
		}
		"Admin"
		{
		...
		"Owner"
		{
			#$permUseAccounts = $permRetrieveAccounts = $permListAccounts = $permAddAccounts = $permUpdateAccountContent = $permUpdateAccountProperties = $permInitiateCPMManagement = $permSpecifyNextAccountContent = $permRenameAccounts = $permDeleteAccounts = $permUnlockAccounts  = $permViewAuditLog = $permViewSafeMembers = $permMoveAccountsAndFolders = $true
			#$permRequestsAuthorizationLevel = 1
			#$permUseAccounts = $permRetrieveAccounts = $permListAccounts = $permUpdateAccountContent = $permInitiateCPMManagement = $permSpecifyNextAccountContent = $permRenameAccounts = $permUnlockAccounts  = $permViewAuditLog = $permViewSafeMembers = $permMoveAccountsAndFolders = $true
			#$permRequestsAuthorizationLevel = 1
			$permUseAccounts = $permRetrieveAccounts = $permListAccounts = $permInitiateCPMManagement = $permUnlockAccounts  = $permViewAuditLog = $permViewSafeMembers = $true
		}
	}
			...
			If (-not $(Test-Safe -safeName $objAccount.safeName -Header $authHeader))
			{
				...
				try
				{
					If ($(Add-Safe -safeName $objAccount.safeName -Header $authHeader))
					{
						...
						Add-SafeOwner -safeName $objAccount.safeName -ownerName "gEPVr_Prod_Administrators" -ownerRole "DefaultAdminGroup" -Header $authHeader
						Add-SafeOwner -safeName $objAccount.safeName -ownerName "gEPVr_Prod_Auditors" -ownerRole "DefaultAuditGroup" -Header $authHeader
						Add-SafeOwner -safeName $objAccount.safeName -ownerName "gEPVr_Prod_SafeManagers" -ownerRole "DefaultSafeManagerGroup" -Header $authHeader
					}
				}
				catch
				{
...
try
{
	If ($personalPrivAccounts.Count -gt 0)
	{
        ...
        ### Modified so that I can delete the creator safemgmt
        
        #foreach ($delsafe in $bulkbody.accountsList.safeName) {
        
        #    $URL_DeleteBaseline = $URL_PVWABaseAPI + "/Safes/" + $delsafe + "/Members/safemgmt"
        #    $deleteResult = Invoke-Rest -Command DELETE -URI $URL_DeleteBaseline -Header $authHeader
            # Write-LogMessage -type Info -MSG "Result of deleting creator account: $URL_DeleteBaseline $($bulkBody | ConvertTo-Json -Depth 5) $($deleteResult.Status -creplace '([A-Z])','$1')"
        #    Write-LogMessage -type Info -MSG "Result of deleting creator account: $URL_DeleteBaseline $($deleteResult.Status -creplace '([A-Z])','$1')"
        #}
