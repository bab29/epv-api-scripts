...
|----------|---------|----------------|---------|
| `Get-IdentityHeader` | Main authentication entry point | IdentityUserName, UPCreds, OAuthCreds, ClientId, ClientSecret, PIN, EnableTranscript, ForceNewSession, IdentityTenantURL, PCloudURL | String (Bearer token) |
...
├── Documentation/
│   ├── IMPLEMENTATION-PLAN.md         # 9-step implementation plan
│   ├── ARCHITECTURE-DESIGN.md         # This file - architecture with Mermaid diagrams
...
│   │   ├── Get-IdentityURL.ps1
│   │   ├── Test-IdentityToken.ps1
│   │   ├── Clear-IdentitySession.ps1
│   │   └── Get-IdentitySession.ps1
│   └── en-US/
│       └── about_IdentityAuth.help.txt
│
├── PS7/                               # PowerShell 7+ module (SOURCE)
│   ├── IdentityAuth7.psm1             # Main module loader (dot-sources all .ps1 files)
│   ├── IdentityAuth7.psd1             # Module manifest
│   ├── Classes/                       # Class definitions
│   │   ├── IdentitySession.ps1
│   │   ├── IdentityAuthResponse.ps1
│   │   ├── ChallengeInfo.ps1
│   │   ├── MechanismInfo.ps1
│   │   ├── SessionManager.ps1
│   │   └── TokenValidator.ps1
│   ├── Enums/                         # Enum definitions
│   │   ├── AuthenticationMechanism.ps1
│   │   ├── ChallengeType.ps1
│   │   ├── MechanismType.ps1
│   │   └── SessionState.ps1
│   ├── Private/                       # Private helper functions (one function per file)
...
│   └── Publish-Module.ps1             # Publish to PowerShell Gallery (future)
│
├── Distribution/                      # OUTPUT - Generated by Build-Module.ps1
│   ├── IdentityAuth.psm1              # SINGLE FILE - All code combined (PS5.1)
│   ├── IdentityAuth.psd1              # Module manifest
│   ├── IdentityAuth7.psm1             # SINGLE FILE - All code combined (PS7+)
│   ├── IdentityAuth7.psd1             # Module manifest
│   └── README.md                      # End-user documentation
│
├── Tests/                             # Shared test infrastructure
│   ├── Test-IdentityAuthManual.ps1    # Interactive testing menu (works with both versions)
│   ├── Capture-CurrentModuleOutput.ps1 # Baseline capture
│   └── Pester/                        # Future: Pester v5 tests
│       ├── IdentityAuth.Tests.ps1     # PS5.1 tests
│       └── IdentityAuth7.Tests.ps1    # PS7+ tests
│
├── .vscode/                           # VS Code workspace settings
│   ├── settings.json                  # PowerShell formatting rules
│   ├── tasks.json                     # Build/test/format tasks (includes build task!)
│   └── extensions.json                # Recommended extensions
│
└── PSScriptAnalyzerSettings.psd1      # Shared strict analysis rules
```

**Distribution Structure (What End Users Get):**
```
Distribution/
├── IdentityAuth.psm1              # SINGLE FILE - Ready for PS 5.1 (import this!)
├── IdentityAuth.psd1              # Module manifest
├── IdentityAuth7.psm1             # SINGLE FILE - Ready for PS 7+ (import this!)
├── IdentityAuth7.psd1             # Module manifest
└── README.md                      # End-user quick start guide

Usage:
  PS 5.1:  Import-Module .\IdentityAuth.psm1
  PS 7+:   Import-Module .\IdentityAuth7.psm1
```
...
**Structure:** Bearer token string (JWT)

**Example:**
```powershell
$token = Get-IdentityHeader -OAuthCreds $creds -PCloudURL 'https://tenant.cyberark.cloud'
# Returns: 'eyJ0eXAiOiJKV1QiLCJhbGciOiJSUzI1NiIsIng1dCI6...'
```
...
$accounts = Invoke-RestMethod @invokeParams
```

**String Format:**
```
eyJ0eXAiOiJKV1QiLCJhbGciOiJSUzI1NiIsIng1dCI6IjlHbW55RlBraGMzaE91UjIybXZTdmduTG83RSIsImtpZCI6IjlHbW55RlBraGMzaE91UjIybXZTdmduTG83RSJ9.eyJhdWQiOiJodHRwczovL2lkZW50aXR5LmN5YmVyYXJrLmNsb3VkL29hdXRoMi90b2tlbiIsImlzcyI6Imh0dHBzOi8vaWRlbnRpdHkuY3liZXJhcmsuY2xvdWQiLCJpYXQiOjE3MDY0NzY4MDAsImV4cCI6MTcwNjQ4MDQwMCwic3ViIjoiYWRtaW5AY3liZXJhcmsuY2xvdWQiLCJzY29wZSI6ImFsbCJ9.signature_here

### Compatibility with Accounts_Onboard_Utility.ps1

The Accounts_Onboard_Utility.ps1 script accepts `$logonToken` parameter as the Bearer token string:
...
The module returns a simple token string compatible with all Privilege Cloud scripts in the epv-api-scripts repository.
